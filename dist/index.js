...
/* harmony import */ var _pause_client__WEBPACK_IMPORTED_MODULE_1__ = __nccwpck_require__(2338);
...
async function run() {
    ...
    const directory = _actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput('directory');
    const client = new _pause_client__WEBPACK_IMPORTED_MODULE_1__/* .PAUSEClient */ .w(username, password);
    const upload = await client.upload({
        file,
        filename,
        directory,
    });
    _actions_core__WEBPACK_IMPORTED_MODULE_0__.setOutput('download-url', upload.downloadUrl());
    _actions_core__WEBPACK_IMPORTED_MODULE_0__.setOutput('metacpan-url', upload.metacpanUrl());
}
try {
    await run();
}
...
;// CONCATENATED MODULE: ./lib/cpan-distname-info.js
const urlRe = /^(?:(?:(?:((?:https?|ftp):\/\/([^/]+)(?=\/).*?\/|file:\/\/.*?\/)?authors\/)?id\/)?([A-Z])\/(\3[A-Z])\/(\4[-A-Z0-9]*)\/)?(.*\/)?([^/]+)$/;
const extRe = /^(.+)\.(tar\.(?:g?z|bz2)|zip|tgz)$/;
const versionRe = /^((?:[-+.]*(?:[A-Za-z0-9]+|(?<=\D)_|_(?=\D))*(?:[A-Za-z](?=[^A-Za-z]|$)|\d(?=-))(?<![._-][vV]))+)(.*)/;
const perlDevRe = /^perl-?(\d+)\.(\d+)(?:\D(\d+))?(-(?:TRIAL|RC)\d+)?$/;
var Maturity;
(function (Maturity) {
    Maturity["Developer"] = "developer";
    Maturity["Released"] = "released";
})(Maturity || (Maturity = {}));
class CPANUpload {
    constructor(url) {
        ...
        if (version.length !== 0) {
            ...
            if (perlDevMatch != null) {
                ...
                if (version.match(/\d\D\d+_\d/) != null ||
    ...
    downloadUrl(mirror) {
        var _a;
        mirror !== null && mirror !== void 0 ? mirror : (mirror = (_a = this.mirror) !== null && _a !== void 0 ? _a : 'https://www.cpan.org/');
...
//# sourceMappingURL=cpan-distname-info.js.map
;// CONCATENATED MODULE: ./lib/pause-client.js
...
async function getRequestData(form) {
    const getLength = (0,external_util_.promisify)(form.getLength.bind(form));
    const headers = {
        ...form.getHeaders(),
...
class PAUSEClient {
    constructor(username, password) {
        ...
        _PAUSEClient_client.set(this, void 0);
        this.uploadUrl = 'https://pause.perl.org/pause/authenquery?ACTION=add_uri';
        const credentials = new auth.BasicCredentialHandler(username, password);
        __classPrivateFieldSet(this, _PAUSEClient_client, new lib.HttpClient('perl-actions/cpan-upload', [credentials]), "f");
    }
    async upload(filedata) {
        var _a;
        const form = new (form_data_default())();
        form.append('HIDDENNAME', this.username);
        form.append('SUBMIT_pause99_add_uri_httpupload', ' Upload this file from my disk ');
        ...
        const filename = (_a = filedata.filename) !== null && _a !== void 0 ? _a : external_path_.basename(filedata.file);
        form.append('pause99_add_uri_upload', await (0,promises_namespaceObject.readFile)(filedata.file), {
            filename,
            contentType: 'application/gzip',
        });
        const res = await __classPrivateFieldGet(this, _PAUSEClient_client, "f").post(this.uploadUrl, ...(await getRequestData(form)));
        const code = res.message.statusCode;
        if (code === 200) {
            const body = await res.readBody();
            if (body.match(/Query succeeded/) == null) {
                throw new Error(`Unknown error uploading ${filename} for user ${this.username}`);
            }
        }
        else if (code === 409) {
            ...
            let err = code !== null && code !== void 0 ? code : 'Unknown';
            if (res.message.statusMessage != null) {
                err = `${err} ${res.message.statusMessage}`;
            }
            throw new Error(`Unknown error "${err}" uploading ${filename} for user ${this.username}`);
        }
...
_PAUSEClient_client = new WeakMap();
//# sourceMappingURL=pause-client.js.map
